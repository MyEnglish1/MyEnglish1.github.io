...
// Получаем элемент с классом "promo__item_link"
var promoItemLink = document.querySelector('.promo__item_link');

// Устанавливаем начальное время
var startTime = new Date();

// Функция для проверки времени и отправки уведомления
function checkTime() {
  // Получаем текущее время
  var currentTime = new Date();

  // Вычисляем разницу во времени в минутах
  var timeDiff = (currentTime - startTime) / 1000 / 60;

  // Если прошло больше 1 минуты
  if (timeDiff > 1) {
    // Получаем текст уведомления из элемента promo__item_link
    var lessonName = promoItemLink.textContent;
    ...
    // Проверяем, есть ли разрешение на отправку уведомлений
      ...
      var notification = new Notification('Вам необходимо повторить урок:', {
        body: lessonName
      });
    }
    ...
    // Если разрешение не получено, запрашиваем его у пользователя
      ...
      Notification.requestPermission().then(function (permission) {
        // Если пользователь дал разрешение, создаем и отображаем уведомление
...
// Вызываем функцию checkTime каждую минуту
